...
public sealed class MainMenuScene : IScene
{
    ...
    public IScene? Update(SceneResources resources, OpenTyrian.Platform.InputSnapshot input, double deltaSeconds)
    {
        ...
        if (confirmPressed && _menuState.SelectedItem.IsEnabled)
        {
            ...
            {
                "one_player_full_game" => new EpisodeSelectScene(),
                _ => null,
            };
        }
    ...
    public void Render(IndexedFrameBuffer surface, SceneResources resources, double timeSeconds)
    {
        ...
        {
            TitleScreenRenderer.RenderMenuOverlay(surface, resources.FontRenderer, CreateDefinition(resources.GameplayText), _menuState);
        }
    }
    ...
    private void EnsureMenuState(SceneResources resources)
    {
        ...
        _menuState = new MenuState(CreateDefinition(resources.GameplayText));
    }

    private static MenuDefinition CreateDefinition(GameplayTextInfo? gameplayText)
    {
        IReadOnlyList<string> names = gameplayText?.GameplayNames ??
            ["Select Game Mode", "1 Player Full Game", "1 Player Arcade", "2 Player Arcade", "Network Game"];
        IReadOnlyList<string> help = gameplayText?.MainMenuHelp ??
