param(
    [string]$OutputDir = "temp\\compare-jukebox"
)
...
function Send-KeySequence {
    ...
    foreach ($key in $Keys) {
        ...
        $extendedFlag = if ($key -in @("Down", "Up", "Left", "Right")) { 0x01000000 } else { 0 }
...
function Invoke-JukeboxCapture {
    param(
        [string]$Name,
    ...
    try {
        ...
        Send-KeySequence -Process $process -Shell $shell -Keys @("Enter") -DelayMilliseconds 2200
        Save-WindowScreenshot -Process $process -Path (Join-Path $appDir "01-title.png")

        Send-KeySequence -Process $process -Shell $shell -Keys $TitleKeys -DelayMilliseconds 1400
        Save-WindowScreenshot -Process $process -Path (Join-Path $appDir "02-title-setup.png")

        Send-KeySequence -Process $process -Shell $shell -Keys @("Enter") -DelayMilliseconds 2200
        Save-WindowScreenshot -Process $process -Path (Join-Path $appDir "03-setup.png")

        Send-KeySequence -Process $process -Shell $shell -Keys $SetupKeys -DelayMilliseconds 1400
        Save-WindowScreenshot -Process $process -Path (Join-Path $appDir "04-setup-jukebox.png")

        Send-KeySequence -Process $process -Shell $shell -Keys @("Enter") -DelayMilliseconds 2200
        Save-WindowScreenshot -Process $process -Path (Join-Path $appDir "05-jukebox.png")

        Send-KeySequence -Process $process -Shell $shell -Keys @("Right") -DelayMilliseconds 1600
        Save-WindowScreenshot -Process $process -Path (Join-Path $appDir "06-jukebox-next.png")
    }
    finally {
...
Invoke-JukeboxCapture -Name "original-c" -ExecutablePath $originalExe -WorkingDirectory (Split-Path -Parent $originalExe) -OutputRoot $outputRoot -TitleKeys @("Down", "Down", "Down", "Down") -SetupKeys @("Down", "Down")
Invoke-JukeboxCapture -Name "csharp-port" -ExecutablePath $dotNetExe -WorkingDirectory (Split-Path -Parent $dotNetExe) -OutputRoot $outputRoot -TitleKeys @("Down", "Down", "Down", "Down", "Down") -SetupKeys @("Down", "Down", "Down")

Write-Host "Jukebox screenshots written to $outputRoot"
